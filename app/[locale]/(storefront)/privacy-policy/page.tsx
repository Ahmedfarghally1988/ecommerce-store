...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const titleKey = locale === 'ar' ? 'privacy_policy_title_ar' : 'privacy_policy_title_en';
  const titleSetting = await prisma.setting.findUnique({ where: { key: titleKey } });
  const customTitle = titleSetting?.value;

  return {
    title: customTitle || t('privacyPolicy'),
  };
}
...
export default async function PrivacyPolicyPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const settingKey = locale === 'ar' ? 'privacy_policy_ar' : 'privacy_policy_en';
  const titleKey = locale === 'ar' ? 'privacy_policy_title_ar' : 'privacy_policy_title_en';
  ...
  const displayTitle = titleSetting?.value || t('privacyPolicy');
  ...
  return (
    <div className="bg-white dark:bg-gray-900 min-h-screen py-6">
      <div className="container mx-auto px-4">
        <Breadcrumb
          locale={locale}
          items={[{ label: displayTitle, href: '/privacy-policy' }]}
        />
